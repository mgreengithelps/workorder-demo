<!--
Component to control Steps in PMs
-->
<template>
  <v-col
    cols="12"
...
// Check to see if name is unique in array
...
export default {
  ...
  methods: {
    save () {
      this.$v.$touch()
      if (!this.$v.$invalid) {
        // Use bus to emit message to parent component
        this.$bus
          .$emit('saveCustom', {
    ...
    deleteThis () {
      this.edit = false
      // Use bus to tell parant to delete the step
      this.$bus
